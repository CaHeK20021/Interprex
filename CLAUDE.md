...
`UnknownExport`). This affects ALL base-game imports (parent classes, outer
objects), not just Ingredients. Root cause: Satisfactory doesn't ship a
`ScriptObjects` chunk in any `.utoc`, so retoc cannot resolve script imports
(class names from /Script/FactoryGame etc.). The `--no-script-objects` flag is
required for retoc to produce output at all (without it, 537/537 assets fail);
with it, UnknownExport stubs are created for every unresolved base-game import.
`Buildable`/`Recipe` (mod-owned) DO resolve. So a full-array CDO replace would
blank `Ingredients` → broken recipe. `SerializeArrayForCdo` therefore OMITS any
field holding an unresolved object (`omitted=True` surfaced). Moot while
...
- **NEVER REBIND a `store` function name — patch it IN PLACE (`__code__` swap).** Ren'Py pickles `store` functions BY REFERENCE on save and verifies identity (`store.<name>` must be the SAME object as the one captured in the save graph). If you do `globals()['fn'] = wrapper` (a NEW object), every save raises `PicklingError: Can't pickle <function fn>: it's not the same object as store.fn` and the player loses ALL progress — saving is impossible. This was a real shipped bug (Killer Chat 1.4.1, `add_ping_hyperlinks`). The fix: keep the SAME function object and swap its behaviour —
  ...
  ```
  The wrapper must have NO closure free variables (capture everything via default args), or `__code__` assignment fails.
  - **GOTCHA (3rd shipped save crash): rebinding `renpy.translation.translate_string` is NOT safe — patch it IN PLACE too.** The earlier claim here that "rebinding a `renpy.*` MODULE attribute is fine — engine modules aren't pickled" is WRONG for `translate_string`: the engine captures the ORIGINAL `translate_string` object into the SAVE GRAPH (a Say's `show_function` → `show_display_say` references it), so pickle stores it BY REFERENCE and verifies `getattr(renpy.translation, 'translate_string')` is the SAME object. A rebind (`renpy.translation.translate_string = wrapper`) makes them differ → `PicklingError: Can't pickle <function translate_string>: it's not the same object as renpy.translation.translate_string` on EVERY save (real shipped Killer Chat 1.4.1 crash). Fix: identical `__code__`-swap as `add_ping_hyperlinks` — clone the original under a UNIQUE name, build a wrapper, assign `__code__`/`__defaults__`/`__kwdefaults__` onto the SAME object, wrap in `try/except: pass`. Because the wrapper's `_orig` default sits after `*args` it's keyword-only, so its default lives in `__kwdefaults__` — copy that too. **2nd crash on the same fix: the CLONE must ALSO be reachable by reference.** The clone lives in `__kwdefaults__['_orig']` and the engine reaches it from the save graph; pickle saves it as `getattr(<__module__>, <__qualname__>)`, and the clone inherits `__module__='renpy.translation'` from the original's globals → `getattr(renpy.translation, '_interprex_orig_translate_string')` raises `attribute lookup ... failed` unless we register the SAME object: `renpy.translation._interprex_orig_translate_string = _interprex_orig_translate_string`. (`add_ping_hyperlinks`'s clone needed no such line — it's a `store` var, found via `getattr(store, …)`; this clone is on a module.) Verified by `check_renpy_font` (the mock `translate_string` is freevar-free so the in-place swap actually runs; the test now mirrors pickle-by-reference for BOTH the patched fn AND the clone — `getattr(module, clone.__qualname__) is clone`, confirmed to fail without the registration line). The rule generalizes: ANY engine function reachable from the rollback log / save graph must be patched in place AND, if a renamed clone is kept, that clone must be resolvable by `getattr(its module/store, its __qualname__)` to itself — module-vs-store is not the deciding factor; reachability-by-pickle is.
  ...
  - **GOTCHA (4th save crash — custom getters/setters in properties):** Any helper getter/setter functions (like `_get_role_name`, `_set_role_name`, `_get_dominant_role`, `_set_dominant_role`, `_get_who_typing_translated`) dynamically added to `ServerRole`, `ChatCharacter`, or `ChatChannel` properties must be registered in the module-level globals of the store: e.g. `globals()['_get_role_name'] = _get_role_name`. Because these classes are serialized during savegame rollback/pickling, pickle attempts to resolve these helper functions by reference at the module level. If they are not registered in the store `globals()`, loading a savegame crashes with `AttributeError: Can't get attribute '_get_role_name'`.

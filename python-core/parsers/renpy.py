...
class RenPyParser(BaseParser):
    ...
    def _write_native_font(self, root: str, lang: str, font_name: str,
        ...
        content = (
            ...
            "#\n"
            "# PICKLE SAFETY: _set_dominant_role, _get_dominant_role, _set_role_name,\n"
            "# _get_role_name are defined inside the translate-python block below and\n"
            "# registered in globals() (= Ren'Py store). Ren'Py pickles store functions\n"
            "# BY REFERENCE on save. When a save is loaded, pickle calls\n"
            "# getattr(store, '_set_dominant_role') BEFORE the translate block runs\n"
            "# (language change happens after load) → AttributeError → crash.\n"
            "# Fix: register identity stubs in store via init python (runs at startup,\n"
            "# unconditionally, before any save is loaded). The translate block below\n"
            "# overwrites them with the real implementations when the language activates.\n"
            "init python:\n"
            ...
            f"                globals()['_get_role_name'] = _get_role_name\n"
            f"                globals()['_set_role_name'] = _set_role_name\n"
            ...
            f"                globals()['_get_dominant_role'] = _get_dominant_role\n"
            f"                globals()['_set_dominant_role'] = _set_dominant_role\n"
            ...
            f"                globals()['_get_who_typing_translated'] = _get_who_typing_translated\n"
